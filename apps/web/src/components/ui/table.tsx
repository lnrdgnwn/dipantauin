...
const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn(
        "h-10 px-4 text-left align-middle text-[12px] font-medium text-muted-foreground [&:has([role=checkbox])]:pr-0 uppercase font-mono caption-mono",
        className
      )}
      {...props}
    />
...
const TableCell = React.forwardRef<HTMLTableCellElement, React.TdHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <td
      ref={ref}
      className={cn("p-4 align-middle text-sm [&:has([role=checkbox])]:pr-0 body-sm", className)}
      {...props}
    />
  )
)
